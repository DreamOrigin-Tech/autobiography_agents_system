...
export default function InterviewChatPage() {
  ...
  const [callModeStatus, setCallModeStatus] = useState("");
  const [callModeError, setCallModeError] = useState("");
  const [callLatestTranscript, setCallLatestTranscript] = useState("");
  const [callDetectedRole, setCallDetectedRole] = useState<InterviewAssistantRole | "">("");
  ...
  const callSliceTimerRef = useRef<number | null>(null);
  ...
  function cleanupCallMode() {
    if (callSliceTimerRef.current !== null) {
      window.clearTimeout(callSliceTimerRef.current);
      callSliceTimerRef.current = null;
    }
  ...
  useEffect(() => {
    return () => {
      ...
      const callRecorder = callRecorderRef.current;
  ...
  async function refreshAssistantGuidance(options?: { silent?: boolean }) {
    if (!options?.silent) setAssistantLoading(true);
    setError("");
    try {
      ...
      if (!options?.silent) setAssistantLoading(false);
    }
  }

  async function startCallAssistantMode() {
    if (callActiveRef.current) return;
    if (!voiceSupport.recording || typeof navigator === "undefined" || !navigator.mediaDevices) {
      setCallModeError("当前浏览器不支持持续录音，可以继续使用文字记录。");
      return;
    }
    setCallModeError("");
    setCallModeStatus("正在请求麦克风权限...");
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        audio: {
          echoCancellation: true,
      ...
      setCallModeStatus("通话采访助理正在听...");
      callStreamRef.current = stream;
      startCallSlice(stream);
    } catch {
      callActiveRef.current = false;
      setCallModeActive(false);
      setCallModeStatus("");
      setCallModeError("麦克风权限没有打开，无法启动通话采访助理。");
    }
  }

  function stopCallAssistantMode() {
    if (!callActiveRef.current && !callRecorderRef.current && !callStreamRef.current) return;
    callActiveRef.current = false;
    setCallModeActive(false);
    setCallModeStatus("通话采访助理已停止");
  ...
  function startCallSlice(stream: MediaStream) {
    if (!callActiveRef.current || callProcessingRef.current) return;
    const mimeType = selectAudioRecordingMimeType();
    let recorder: MediaRecorder;
    try {
      recorder = new MediaRecorder(stream, mimeType ? { mimeType } : undefined);
    } catch {
      setCallModeError("浏览器无法创建录音通道，请刷新页面或换一个浏览器重试。");
      stopCallAssistantMode();
      return;
    }
    const chunks: Blob[] = [];
    ...
    recorder.onerror = () => {
      setCallModeError("持续录音失败，请重新启动通话采访助理。");
      stopCallAssistantMode();
    };
    recorder.onstop = () => {
      ...
      void submitCallAudioSlice(audio);
    };
    recorder.start();
    callSliceTimerRef.current = window.setTimeout(() => {
      callSliceTimerRef.current = null;
      if (recorder.state !== "inactive") recorder.stop();
    }, 6500);
  }

  async function submitCallAudioSlice(audio: Blob) {
    if (!callActiveRef.current) return;
    if (audio.size < 1200) {
      if (callStreamRef.current) startCallSlice(callStreamRef.current);
      return;
    }
    callProcessingRef.current = true;
    setCallModeStatus("正在识别并判断说话人...");
    try {
      const advice = await api.recordInterviewCallAudio(chapterId, audio);
      setAssistantAdvice(advice);
      setCoverage(advice.chapter_coverage);
      setSuggestedAction(advice.suggested_action);
      setCallLatestTranscript(advice.transcript || "");
      setCallDetectedRole(advice.detected_role || "");
      ...
      setCallModeError(e instanceof Error ? e.message : "通话识别失败");
      setCallModeStatus("识别失败，仍在继续听");
    } finally {
      callProcessingRef.current = false;
      if (callActiveRef.current && callStreamRef.current) {
        startCallSlice(callStreamRef.current);
      }
    }
  }
      ...
      <div className="flex min-h-screen min-w-0 flex-1 flex-col">
        ...
        <div className="grid min-h-0 flex-1 md:grid-cols-[minmax(0,1fr)_18rem]">
          <section className="order-2 min-w-0 px-4 py-5 pb-56 sm:px-7 md:order-1 md:max-w-3xl md:justify-self-center md:w-full">
            ...
            {workMode === "assistant" && (
              <div className="mb-5 rounded-xl border border-[#c7d7fe] bg-[#f5f8ff] px-4 py-3">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-[#1d2939]">语音电话式采访助理</p>
                    <p className="mt-1 text-xs leading-relaxed text-[#667085]">持续录音并分段识别，自动判断采访员或受访者讲话，像通话记录一样收集素材。</p>
                  </div>
                </div>
                {(callModeStatus || callLatestTranscript || callModeError) && (
                  <div className="mt-3 rounded-lg bg-white px-3 py-2 text-xs leading-relaxed text-[#475467]">
                    {callModeStatus && <p>{callModeStatus}</p>}
                    {callLatestTranscript && (
                      <p className="mt-1">
                        <span className="font-medium text-[#344054]">{roleLabel(callDetectedRole || "note")}：</span>
                        {callLatestTranscript}
                      </p>
                    )}
                    {callModeError && <p className="mt-1 text-[#b42318]">{callModeError}</p>}
                  </div>
                )}
              </div>
            )}
        ...
        <form onSubmit={handleSend} className="fixed bottom-16 inset-x-0 z-20 border-t border-[#dfe5eb] bg-white/95 px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] backdrop-blur-xl md:bottom-0 md:left-72 md:px-7">
          <div className="mx-auto max-w-3xl">
            {workMode === "assistant" ? (
              <div className="rounded-2xl border border-[#b8dcd7] bg-[#f0fdfa] px-3 py-3 shadow-[0_4px_18px_-12px_rgba(15,118,110,0.55)] sm:px-4">
                <div className="flex flex-wrap items-center gap-3">
                  <div role="tablist" aria-label="采访模式" className="inline-flex shrink-0 rounded-lg border border-[#b8dcd7] bg-white/70 p-1">
                    <button type="button" role="tab" aria-selected={false} onClick={() => { stopCallAssistantMode(); setWorkMode("ai"); setError(""); }} className="rounded-md px-3 py-1.5 text-xs font-medium text-[#667085] transition hover:text-[#1f2937]">
                      AI
                    </button>
                    <button type="button" role="tab" aria-selected={true} onClick={() => { stopVoiceConversation(); setVoiceMode("text"); setWorkMode("assistant"); setError(""); if (!assistantAdvice) void refreshAssistantGuidance({ silent: true }); }} className="rounded-md bg-white px-3 py-1.5 text-xs font-medium text-[#1f2937] shadow-sm transition">
                      助理
                    </button>
                  </div>
                  <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${callModeActive ? "bg-[#0f766e] text-white ai-glow" : "bg-white text-[#0f766e]"}`}>
                    <MicIcon />
                  </div>
                  <div className="min-w-36 flex-1">
                    <p className="text-sm font-semibold text-[#115e59]">{callModeActive ? "通话采访进行中" : "采访助理通话"}</p>
                    <p className="mt-0.5 truncate text-xs text-[#667085]">{callModeStatus || "开启后持续收音，自动区分采访员和受访者"}</p>
                  </div>
                  <button type="button" onClick={callModeActive ? stopCallAssistantMode : startCallAssistantMode} className={`btn-primary inline-flex shrink-0 items-center gap-1.5 px-4 py-2.5 text-sm ${callModeActive ? "bg-[#b42318] hover:bg-[#912018]" : ""}`}>
                    {callModeActive ? <StopIcon /> : <MicIcon />}
                    {callModeActive ? "停止通话" : "开始通话"}
                  </button>
                </div>
                {(callLatestTranscript || callModeError) && (
                  <div className="mt-3 rounded-xl bg-white/80 px-3 py-2 text-xs leading-relaxed text-[#475467]">
                    {callLatestTranscript && (
              ...
              <>
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  <div role="tablist" aria-label="采访模式" className="inline-flex shrink-0 rounded-lg border border-[#dfe5eb] bg-[#f7f8fa] p-1">
                    <button type="button" role="tab" aria-selected={true} onClick={() => { stopCallAssistantMode(); setWorkMode("ai"); setError(""); }} className="rounded-md bg-white px-3 py-1.5 text-xs font-medium text-[#1f2937] shadow-sm transition">
                      AI
                    </button>
                    <button type="button" role="tab" aria-selected={false} onClick={() => { stopVoiceConversation(); setVoiceMode("text"); setWorkMode("assistant"); setError(""); if (!assistantAdvice) void refreshAssistantGuidance({ silent: true }); }} className="rounded-md px-3 py-1.5 text-xs font-medium text-[#667085] transition hover:text-[#1f2937]">
                      助理
                    </button>
                  </div>
                  <div role="tablist" aria-label="回答方式" className="inline-flex shrink-0 rounded-lg border border-[#dfe5eb] bg-[#f7f8fa] p-1">
                    <button type="button" role="tab" aria-selected={voiceMode === "text"} onClick={() => { if (voiceMode === "voice") stopVoiceConversation(); setError(""); setVoiceMode("text"); }} className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition ${voiceMode === "text" ? "bg-white text-[#1f2937] shadow-sm" : "text-[#667085] hover:text-[#1f2937]"}`}>
                      <KeyboardIcon /> 文字
                    </button>
                    <button type="button" role="tab" aria-selected={voiceMode === "voice"} onClick={() => { setError(""); setVoiceMode("voice"); }} className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition ${voiceMode === "voice" ? "bg-white text-[#1f2937] shadow-sm" : "text-[#667085] hover:text-[#1f2937]"}`}>
                      <MicIcon /> 语音
                    </button>
                  </div>
                </div>
                  ...
                  <div className="rounded-2xl border border-[#b8dcd7] bg-[#f0fdfa] px-4 py-3 shadow-[0_4px_18px_-12px_rgba(15,118,110,0.55)]">
                    ...
                    {voiceState.phase === "reviewing" ? (
                      <div className="mt-3 rounded-xl bg-white/80 px-3 py-2">
                        ...
                        <textarea id="voice-transcript-review" value={input} onChange={(event) => setInput(event.target.value)} rows={3} className="input max-h-32 min-h-20 w-full resize-y bg-white text-sm leading-relaxed" />
                      </div>
...
function roleLabel(role: InterviewAssistantRole | "agent" | ""): string {
